...
layout = [
    [sg.Text('Unicode Characters Demo', font='Def 16')],
    [
        sg.T('Network Status', size=(12, 1)),
        sg.Text(
            size=(10, 1), font='Default 18', text_color='green', key='-OUT1-'
        ),
    ],
    [
        sg.T('Gas Level', size=(12, 1)),
        sg.Text(
            size=(10, 1), font='Default 18', text_color='green', key='-OUT2-'
        ),
    ],
    [
        sg.T('Oil Temp', size=(12, 1)),
        sg.Text(
            size=(10, 1), font='Default 18', text_color='blue', key='-OUT3-'
        ),
    ],
    [
        sg.T('Status4', size=(12, 1)),
        sg.Text(
            CIRCLE,
            size=(10, 1),
            font='Default 18',
    ...
    [
        sg.Frame(
            'Display Unicode Characters In Range',
            [
                [
                ...
                [
                    sg.B('Display Chars', bind_return_key=True),
                    sg.T('Display Font Size'),
                    sg.Spin(
                        list(range(10, 25)),
                        initial_value=18,
                        font='Any 14',
    ...
    [
        sg.Multiline(
            size=(30, 10), font='Any 18', key=f'-MLINE-{sg.WRITE_ONLY_KEY}'
        )
    ],
...
while True:
    ...
    if event in (sg.WIN_CLOSED, 'Exit'):  # always,  always give a way out!
        ...
        try:
            for c in range(int(values['-START-']), int(values['-END-'])):
                window[f'-MLINE-{sg.WRITE_ONLY_KEY}'](
                    chr(c), append=True, text_color_for_value='red'
                )
    ...
    if multiline_font_size != int(values['-FONTSIZE-']):
        window[f'-MLINE-{sg.WRITE_ONLY_KEY}'](font='Any '+ str(values['-FONTSIZE-']))
