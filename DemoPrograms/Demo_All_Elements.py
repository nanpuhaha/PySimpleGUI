...
def main():
    ...
    while True:
        ...
        if event in (None, 'Exit'):
            ...
            for i in range(1000):
                print(f"[LOG] Updating progress bar by 1 step ({str(i)})")
            ...
            sg.popup(f"You chose: {str(folder_or_file)}")
            print(f"[LOG] User chose folder: {str(folder_or_file)}")
        elif event == "Open File":
            print("[LOG] Clicked Open File!")
            folder_or_file = sg.popup_get_file('Choose your file')
            sg.popup(f"You chose: {str(folder_or_file)}")
            print(f"[LOG] User chose file: {str(folder_or_file)}")
        elif event == "Set Theme":
            print("[LOG] Clicked Set Theme!")
            theme_chosen = values['-THEME LISTBOX-'][0]
            print(f"[LOG] User Chose Theme: {str(theme_chosen)}")
            window.close()
